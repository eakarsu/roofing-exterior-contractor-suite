const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5480';
...
const adminCookie = await login('admin@roofing-exterior-contractor.local', 'admin123');
const managerCookie = await login('manager@roofing-exterior-contractor.local', 'manager123');
const analystCookie = await login('analyst@roofing-exterior-contractor.local', 'analyst123');
